...
import type { AnalyticsVisuals } from "@/lib/analytics/visuals"
 ...
 *
 * Series are derived on the server and cached — see `lib/analytics/visuals.ts`
 * for why. The panel charts the full ~4,600-institution cohort rather than the
 * screening table already on the client, which is capped at the largest ~1,100
 * nationally; scores are cohort-relative, so charting the capped set would
 * quietly plot a different population from the one the caption claims.
 *
 * Fetching is deferred until the panel is near the viewport. It sits below the
 * table and the KPI cards, so on most screens it is not visible at load, and
 * fetching eagerly made the rest of the tab wait behind it.
 */
export function MarketAnalyticsVisuals({ scope, asOfQuarter }: { scope: string; asOfQuarter: string }) {
  const [data, setData] = useState<AnalyticsVisuals | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const containerRef = useRef<HTMLDivElement>(null)
  const [inView, setInView] = useState(false)

  // 400px of margin so the fetch starts just before the panel is reached and
  // the charts are usually there by the time it is. Where IntersectionObserver
  // is unavailable the panel simply loads immediately, as it did before.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    getAnalyticsVisuals(scope)
      .then((result) => {
        if (!active) return
        if (result.ok) setData(result.visuals)
  ...
  return (
    <Card ref={containerRef} className="p-6 surface-primary">
      <div className="mb-4">
...
function VisualGrid({ data }: { data: AnalyticsVisuals }) {
  const { histogram, creToCapitalRanking, exposureMix, scatter } = data

  return (
    <div className="grid gap-4 lg:grid-cols-2 animate-in fade-in duration-500">
      <Panel
        title="Opportunity Score Distribution"
        caption={`How ${data.institutionCount} institutions spread across the score bands`}
      >
