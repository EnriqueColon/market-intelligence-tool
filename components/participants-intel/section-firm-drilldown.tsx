...
import type { FlowEdge, FlowWindowStats } from "@/lib/participants-intel/types"
...
export function SectionFirmDrilldown({ selectedFirm, rolling, edges }: Props) {
  if (!selectedFirm) {
    return (
  ...
  return (
    <Card className="p-6 border-slate-200/80 bg-slate-50/30">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <div className="rounded border border-slate-200 bg-white p-3 text-sm">Inbound 30d: <strong>{money(row?.inbound30d || 0)}</strong></div>
        <div className="rounded border border-slate-200 bg-white p-3 text-sm">Outbound 30d: <strong>{money(row?.outbound30d || 0)}</strong></div>
        <div className="rounded border border-slate-200 bg-white p-3 text-sm">Net Position (30d): <strong>{money(row?.net30d || 0)}</strong></div>
